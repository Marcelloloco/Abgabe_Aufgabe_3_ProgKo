...
Mat openMPapplyGreyscale(Mat image);
Mat openMPconvertBGR2HSV(Mat image);
Mat openMPmyOwnEmboss(Mat image);
...
double openMPtimes[rounds];

int openMPfilter[filter_width][filter_height] =
{
    -1, -1, 0,
    -1, 0, 1,
    0, 1, 1
...
int main(int argc, char const* argv[])
{
    ...
    for (int i = 0; i < input_size; i++)
    {
        ...
        imwrite("out/greyscale/" + inputs[i], openMPapplyGreyscale(image));
        imwrite("out/hsv/" + inputs[i], openMPconvertBGR2HSV(image));
        imwrite("out/emboss/" + inputs[i], openMPmyOwnEmboss(image));
    ...
    openMPtimes[round] = duration.count();
    ...
    std::cout << "DURATION: " << time/rounds << std::endl;
